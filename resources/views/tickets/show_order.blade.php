...
<div class="row">
    <div class="col-lg">
        ...
        <div class="card">
            ...
            <div class="card-body">
                {{ Form::open(['action' => ['TicketsController@store', $ticket->id], 'method' => 'POST']) }}
                <h4>Počet vstupenek</h4>
                <div class="quantityContainer row p-4">
                    <span class="minus btn btn-secondary col-1 mr-2">-</span> {{Form::number('quantityInput', $quantity,
                    ['id' => 'quantityInput', 'class' => 'form-control input-number text-center col-2', 'placeholder' =>
                    '1', 'required', 'min' => '1', 'max' => '10'])}}
                ...
                <div class="form-check">
                    {{Form::radio('payment_method',$method->id, false, ['class' => 'form-check-input', 'id' => 'method_'.$method->id])
                    }} {{Form::label('method_'.$method->id, $method->name, ['class' => 'form-check-label'])}}
                </div>
                @endforeach
                <hr> {{Form::label('user_message', 'Zde můžete zanechat zprávu prodejci:')}} {{Form::textarea('user_message',
                '', ['class' => 'form-control', 'placeholder' => 'Zpráva prodejci', 'maxlength' => '255', 'rows' => '3'])}}
                {{Form::submit('Koupit', ['class'=>'btn btn-primary buy mt-3'])}} {{ Form::close() }}
            </div>
        </div>
